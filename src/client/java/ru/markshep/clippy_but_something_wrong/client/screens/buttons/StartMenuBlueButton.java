...
public class StartMenuBlueButton extends ClickableWidget {
    ...
    public StartMenuBlueButton(int x, int y, Identifier texture, String text, Runnable onClickAction) {
        super(x, y, 20 + client.textRenderer.getWidth(text) + 1, 16 + 1, Text.of(text));
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        if (this.isHovered()) {
            ...
            context.fill(this.getX() - 1, this.getY() - 1, this.getX() + this.getWidth() + 1, this.getY() + this.getHeight(), 0xFF0052CC);

            context.drawTexture(RenderLayer::getGuiTextured, this.texture, this.getX(), this.getY(), 0, 0, 16, 16, 16, 16);

            context.drawText(client.textRenderer, this.getMessage(), this.getX() + 18, this.getY() + 4, 0xFFFFFF, true);

        } else {

            context.drawTexture(RenderLayer::getGuiTextured, this.texture, this.getX(), this.getY(), 0, 0, 16, 16, 16, 16);

            context.drawText(client.textRenderer, this.getMessage(), this.getX() + 18, this.getY() + 4, 0x696969, true);
